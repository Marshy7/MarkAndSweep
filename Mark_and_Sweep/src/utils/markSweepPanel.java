...
public class markSweepPanel extends JPanel {
	 ...
	 * Create the panel.
	 */
	public markSweepPanel(garbageCollector gc, int width, int height) {
	...
	public void reDraw(){
		...
		for(int i = 0; i < handlePoolSize; i++){
			if(gc.handlePool[i] != null){
				...
				if(gc.handlePool[i].mark){
					fishColor = Color.GRAY;
				}
				else{
					fishColor = gc.handlePool[i].fishType.myColor;
				}
				
				draw.setPenColor(fishColor);
